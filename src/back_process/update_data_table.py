...
def update_Cadence(ID: str) -> None:
    workbook = openpyxl.load_workbook(PATH_EXCEL)
    sheet = sheet = workbook[workbook.sheetnames[0]]

    value_subject = pd.read_excel(PATH_EXCEL, index_col=0).loc[ID]
    ...
    for index_shoe, shoe in enumerate(order_shoes.keys()):
        ...
        if name == 'ascis':
            name = 'asics'
        path_pdf = []
        for dirpath, subdirs, files in os.walk(os.path.join(PATH_PRESSURE_REPORT, ID)):
